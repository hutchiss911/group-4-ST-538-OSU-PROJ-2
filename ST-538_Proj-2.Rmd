...
knitr::opts_chunk$set(fig.width = 6, fig.height = 4, size = "small", family = "serif", face = "bold")
...
Initially, we allocated individual tasks to each team member and emphasized focusing on their assigned responsibilities. However, we noticed variations in the approach to problem-solving among team members. During collaboration on this project using Git, conflicts arose when pushing changes to the main branch. To ensure both obstacles were dealt with appropriately, we reached an agreement stipulating that each team member must have their changes reviewed and approved by the next person before merging them into the main branch.
